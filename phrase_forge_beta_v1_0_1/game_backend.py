...
from lexicon.validators import DEFAULT_PROFILE, available_profiles, lexicon_info, word_allowed
from solver.indexed_solver import indexed_candidates, index_stats
...
def is_valid_english_word(word: str, min_zipf: float = 2.0, profile: Optional[str] = None) -> bool:
    """Return whether the word is accepted by the Phrase Forge Lexicon.

    ``min_zipf`` is retained for backward compatibility; PFL profile thresholds
    are authoritative in Beta 1.0.
    """
    return word_allowed(normalize_word(word), profile or DEFAULT_PROFILE)


def get_word_info(word: str, profile: Optional[str] = None) -> dict:
    """Expose PFL metadata for gameplay explanations and Admin diagnostics."""
    return lexicon_info(normalize_word(word), profile or DEFAULT_PROFILE)
...
def solver_index_stats(max_words: int = 200_000) -> dict:
    return index_stats(max_words)
...
def grade_solution(
    ...
    lexicon_profile: Optional[str] = None,
) -> dict:
    ...
    if require_english and not is_valid_english_word(sol, profile=lexicon_profile):
        info = get_word_info(sol, lexicon_profile)
        reason_map = {
            "pronoun": "Pronouns are not accepted in the Phrase Forge Lexicon.",
            "proper_name": "Proper names are not accepted in the Phrase Forge Lexicon.",
            "profile_excluded": "This word is not accepted in the selected Phrase Forge Lexicon profile.",
            "too_rare_for_profile": "This word is too rare for the selected Phrase Forge Lexicon profile.",
            "dictionary_unavailable": "The English dictionary source is unavailable in this environment.",
        }
        return {
            "ok": False, "solution": sol, "raw_input": raw,
            "reason": reason_map.get(info.get("reason"), "Not recognized by the Phrase Forge Lexicon."),
            "starts_with_vowel": vowel, "lexicon": info,
        }
    ...
    return {
        ...
        "score_base": score,
        "score_final": score,
        "lexicon": get_word_info(sol, lexicon_profile) if require_english else None,
    }
...
def explain_answer_attempt(
    ...
    dictionary_ok = bool(normalized) and (not require_english or is_valid_english_word(normalized, profile=lexicon_profile))
...
def _cached_solution_tuple(
    word1: str,
    word2: str,
    phrase_label: str,
    min_consonant_len: int,
    require_english: bool,
    max_words: int,
    lexicon_profile: str,
) -> Tuple[Tuple[tuple, ...], ...]:
    puzzle = Puzzle(word1, word2, phrase_label)
    candidate_words = indexed_candidates(word1 + word2, min_consonant_len, max_words=max_words)
    results: List[dict] = []
    for word in candidate_words:
        grade = grade_solution(
            puzzle, word, min_consonant_len, require_english,
            lexicon_profile=lexicon_profile,
        )
        if grade.get("ok"):
            results.append(grade)
    results.sort(key=lambda x: (x.get("score_base", 0), x.get("len", 0), x["solution"]), reverse=True)
    # Cache an immutable representation to avoid cross-session mutation.
    return tuple(tuple(sorted(item.items(), key=lambda kv: kv[0])) for item in results)
...
def all_valid_solutions(
    ...
    consonant_min = max(1, puzzle.total_len - 2) if min_consonant_len is None else max(1, int(min_consonant_len))
    profile = (lexicon_profile or DEFAULT_PROFILE).lower()
    packed = _cached_solution_tuple(
        puzzle.word1, puzzle.word2, puzzle.phrase_label, consonant_min,
        bool(require_english), int(max_words), profile,
    )
    results = [dict(items) for items in packed]
    return results[: max(1, int(limit))]


def clear_solution_cache() -> None:
    _cached_solution_tuple.cache_clear()
...
def build_validated_hint_candidates(
    ...
    candidates = all_valid_solutions(
        puzzle, min_letters_used, require_english, limit=limit,
        lexicon_profile=lexicon_profile,
    )
    return [
        result for result in candidates
        if grade_solution(
            puzzle, result["solution"], min_letters_used, require_english,
            lexicon_profile=lexicon_profile,
        ).get("ok")
    ]
...
def compute_difficulty(
    ...
    solutions = all_valid_solutions(
        puzzle, min_consonant_len, True, max_words=max_words, limit=5000,
        lexicon_profile=lexicon_profile,
    )
    scores = [int(item.get("score_base", 0)) for item in solutions]
    return {
        "solutions_found": len(solutions),
        "tier": difficulty_from_solution_count(len(solutions)),
        "capped_at": 5000,
        "longest_solution": max((len(item["solution"]) for item in solutions), default=0),
        "highest_base_score": max(scores, default=0),
        "average_base_score": round(sum(scores) / len(scores), 1) if scores else 0.0,
        "lexicon_profile": (lexicon_profile or DEFAULT_PROFILE).lower(),
    }
